...
- Hardcoded secrets: run `grep -rE "(api_key|secret|password|token)\s*=\s*['\"][^'\"]{8,}"` on changed files
...
- **20 to 100 files**: read the diff first, then identify and deep-read high-risk files — auth, payment, config, migration, and files touching shared utilities
